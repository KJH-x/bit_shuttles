...
async function poll() {
  ...
  if (isHidden()) {
    clearTimeout(timer);
    timer = setTimeout(poll, HIDDEN_CHECK_MS);
    return;
  }
  inFlight = true;
  try {
...
// 某日的彩虹班次 → 班次列表展示项（未来班次视图用；school 源不含彩虹，故单独并入）。
// priceOf(route, dep) 可注入（优先用静态表价格保持一致），回退实车 presentPrice。
export function rainbowTripsForDate(trips, dateStr, priceOf) {
  const out = [];
  for (const t of trips || []) {
    if (!t || t.serviceDate !== dateStr || !t.boardRoute || !t.dep) continue;
    const staticPrice = priceOf ? priceOf(t.boardRoute, t.dep) : null;
    const real = t.price != null && t.price !== "" ? `¥${Number(t.price).toFixed(2)}` : null;
    out.push({
      id: `rb-${t.planId}`,
      route: t.boardRoute,
      dep: t.dep,
      rainbow: true,
      price: staticPrice || real || "¥10.00"
    });
  }
  return out;
}
