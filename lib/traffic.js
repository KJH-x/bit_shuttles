...
import { trafficDirForRoute, REV_ROUTE } from "./traffic-routes.js";
...
export function trafficForRoute(data, route) {
  ...
  const dir = trafficDirForRoute(route);
  if (!dir) return null;
  return data.dirs[dir] || null;
}
...
export function laneGradient(segments, route) {
  ...
  if (!(total > 0)) return "";
  const order = route === REV_ROUTE ? segments.slice().reverse() : segments;
  const stops = [];
  let cum = 0;
...
// fetch/interval 由本模块管理，返回清理函数。
let trafficCb = null;
let trafficTimer = null;

function pollTraffic() {
  if (!trafficCb || isHidden()) return;
  fetch("/api/traffic", { cache: "no-store" })
    .then((r) => (r.ok ? r.json() : null))
    .then((data) => trafficCb(normalizeTraffic(data)))
    .catch(() => trafficCb(normalizeTraffic(null)));
}
...
export function initTraffic(cb) {
  ...
  const offVisible = onVisible(() => pollTraffic());
  return () => {
    offVisible();
    trafficCb = null;
...
// 用户点「刷新」：立即重拉一次实时路况（不等待下个轮询节拍）
export function refreshTrafficNow() {
  pollTraffic();
}
