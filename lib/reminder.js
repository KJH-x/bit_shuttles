...
//   bitbus-reminder-asked       = "1"（一次性引导「你刚刚触发了抢票提醒」只显示一次，设置之后不再弹；
//                                 未启用时点击班次无反应，此标记持久化于 localStorage）
...
export const ASKED_KEY = "bitbus-reminder-asked";
...
export function hasBeenAsked() {
  ...
  // 兼容旧用户：已配置过（askedOnce=true）视为已询问过，不重复弹引导
  const p = safeGet(PREF_KEY, null);
  return !!(p && p.askedOnce === true);
}
