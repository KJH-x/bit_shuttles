...
export function campusStopAt(trip, now, campus) {
  ...
  let stop = null;
  for (const b of campus.board) {
    if (relMin < b.until) { stop = b.name; break; }
    stop = b.name;
  }
  return relMin < LEAVE_CAMPUS_MIN ? stop : null;
}
...
export function tripLocation(trip, now, checkpoints, campus) {
  ...
  if (now >= trip.arrMs) {
    return campus && Array.isArray(campus.arrive)
      ? { kind: "arrived", text: `${trip.destLabel || "到达"} · ${campus.arrive.join("→")}` }
      : { kind: "arrived", text: "已到达" };
  }
  if (now < trip.depMs) return { kind: "wait", text: "等待发车" };
