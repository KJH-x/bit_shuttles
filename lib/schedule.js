...
// 实时 ETA 与静态理想插值偏差（分钟）：|diff|>3 才显示；dur 或理想缺失返回 null（差 0 也天然不显示）
export function etaDiffMin(trip, profiles) {
  const ideal = lookupDuration(trip.dep, profiles && profiles[trip.route]);
  if (trip.dur == null || ideal == null) return null;
  const diff = Math.round(trip.dur - ideal);
...
export function campusStopAt(trip, now, campus) {
  ...
  if (relMin > leave) return null;
  // 开始上车时同时显示上车点位置（良乡=东校区上车点、中关村=西门上车点）
  const origin = campus.board[0] ? campus.board[0].name : null;
  return origin ? `${BOARDING_LABEL} · ${origin}` : BOARDING_LABEL;
}
...
export function tripLocation(trip, now, checkpoints, campus) {
  ...
  if (next) {
    ...
    // 拆分收费站后缀（cpNote）供 PIDS 窄屏隐藏；无 note 时为 null
    return { kind: "road", text: `距${next.label} 约 ${remainMin}分钟${dist}`, cpName: cp ? cp.name : null, cpNote: cp && cp.note ? cp.note : null };
  }
  return campus && campus.final != null
    ? { kind: "road", text: `距终点 约 ${Math.ceil((trip.arrMs - now) / 60000)} 分钟`, cpName: null, cpNote: null }
    : { kind: "road", text: "运行中", cpName: null, cpNote: null };
}
...
export function ticketInfo(trip, now) {
  ...
  // T-5 起：不再展示「已停止售票」，改为「可买不可退」（售罄前尚可购票，售出不退款）
  return { type: "regular", phase: "closed", label: "可买不可退", saleOpen, saleClose };
}
