...
| v1.8 | 2026-09-02 | **二号屏 FIDS**（route `/FIDS`，`_redirects` 支持；顶部「标准屏 / FIDS」切换，pushState+popstate）：全车次一行一趟、紧凑排列，仅显示方向/开点/状态（等待发车/催促上车/已出发/已到达，文字四色区分；底色三类：未发车 pre / 运行中 run / 已到达 done）；**虚拟站点（checkpoint）**：良乡⇄中关村 途经 京良收费站→杜家坎收费站→六里桥（`CHECKPOINTS`，无实际停靠，`checkpointTimes` 按耗时等分估算，平均分配暂定，待补充精确位置/间距）；标准屏「正在运行」列表与 FIDS 均展示虚拟站点信息 | 当前 |
